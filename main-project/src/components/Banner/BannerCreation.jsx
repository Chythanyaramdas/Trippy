    ...
    <div className='w-3/4 bg-white p-11 rounded-3xl h-3/4 shadow-lg'>
      ...
      >
        ...
        <div className='flex flex-col justify-center mt-16'>
            <input type="text" placeholder='Title' className='p-2 mt-4 w-fit' onChange={(e)=> setTitle(e.target.value)} value={title} />
            <input type="file" name="image" className='p-2 mt-4 w-fit'  onChange={(e)=> setImage(e.target.files[0])} />
            <img className='w-96 p-2 mt-4' src={image?URL.createObjectURL(image):""} alt=""  />
            <input type="text" placeholder='Description' className='p-2 mt-4 w-fit' onChange={(e)=> setDescription(e.target.value)} value={description } />
